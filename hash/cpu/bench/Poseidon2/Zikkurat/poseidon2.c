...
void *my_thread_fun(void *ptr) { 
  ...
  poseidon_merkle_std( thread_data->depth, thread_data->input, thread_data->tgt );
/*
  char s[32];
  sprintf(s, "> thread #%d -> root = ", thread_data->tidx);
  print_std(s, thread_data->tgt);
*/
  pthread_exit(NULL);
} 
...
void merkle_root_multithread(int n, int nThreads) {
  int N = (1<<n);
  ...
  uint64_t   *subtree_roots = malloc( 8*NLIMBS          *nThreads );
  ...
  for(int t=0; t<nThreads; t++) {
    ...
    thread_data[t].depth = n - logNThreads;
    thread_data[t].tgt   = subtree_roots + NLIMBS*t;
    thread_data[t].input = input + (N/nThreads)*NLIMBS*t;
  ...
  uint64_t root[NLIMBS];
  poseidon_merkle_std( logNThreads, subtree_roots, root );
